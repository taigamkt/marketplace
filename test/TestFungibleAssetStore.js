...
contract("AssetType", async function(accounts) {
  ...
  describe('FungibleAssetStore - New Asset Type', () => {
    ...
    it("should create a new asset type and verify the tokens are created", async function() {
      ...
      let tx = await contract.createAssetType(title, description, imageUrl, assetType, supply, {from: owner});
    ...
    it("should create a new asset type and transfer to another account", async function() {
      ...
      await contract.createAssetType(title, description, imageUrl, assetType, supply, {from: owner});
    ...
    it("should get the tokens owned by index", async function() {
      ...
      await contract.createAssetType(title, description, imageUrl, assetType, supply, {from: owner});
      await contract.createAssetType(title1, description, imageUrl, assetType, supply, {from: owner});
      await contract.createAssetType(title2, description, imageUrl, assetType, supply, {from: owner});

      await contract.transfer(firstPlayer, 0, {from:owner});
      await contract.transfer(firstPlayer, 10, {from:owner});
      await contract.transfer(firstPlayer, 23, {from:owner});

      let balance = await contract.balanceOf.call(firstPlayer);
      assert.equal(balance, 3, "Balance is not correct");
      ...
      assert.equal(asset0[0], title, "Title first asset not correct");
      assert.equal(asset1[0], title1, "Title second asset not correct");
      assert.equal(asset2[0], title2, "Title third asset not correct");
