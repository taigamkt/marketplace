...
contract FungibleAssetStore is ERC721Token, Ownable {
  ...
  */
  ...
  function createAssetType(string _title, string _description, string _imageUrl, string _assetType, uint _supply) public{
  ...
  function getAssetType(uint _assetTypeId) public view returns (string title, string description, string imageUrl, string assetType, uint supply) {
    require(_assetTypeId <= assetTypes.length);
    FungibleAssetType storage asset = assetTypes[_assetTypeId-1];
    return (asset.title, asset.description, asset.imageUrl, asset.assetType, asset.supply);
  }

  function getAssetTypeFromTokenId(uint _tokenId) public view returns (string title, string description, string imageUrl, string assetType, uint supply) {
    uint assetTypeId = tokenIdAssetTypeId[_tokenId];
    return getAssetType(assetTypeId);
  }

  function getAssetTypeFromIndex(address _owner, uint _index) public view returns (string title, string description, string imageUrl, string assetType, uint supply) {
    uint[] memory tokenIds = tokensOf(_owner);
    require(_index < tokenIds.length);
    uint tokenId = tokenIds[_index];
    uint assetTypeId = tokenIdAssetTypeId[tokenId];
    return getAssetType(assetTypeId);
